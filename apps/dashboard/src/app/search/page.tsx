...
function Search() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const results = await services.properties.searchProperties(query) as unknown as Property[]

        setSearchResults(results)

    } catch (err) {
        setError("Failed to execute property search. Check the Searching Agent backend.")
    } finally {
  ...
  return (
    <div className="p-4 sm:p-8 space-y-8">
      ...
      <form onSubmit={handleSearch} className="bg-white shadow-lg rounded-lg p-6 space-y-4">
      ...
      {/* Results Section */}
      <h2 className="text-2xl font-semibold text-gray-800">Search Results ({searchResults.length})</h2>
