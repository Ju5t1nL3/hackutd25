...
const CustomerInfoPanel: React.FC<CustomerInfoPanelProps> = ({
      ...
      <div className="h-full flex items-center justify-center bg-white">
        <div className="text-center text-gray-400">
          ...
          <p className="text-xs mt-1">
            Select a call to view customer information
          </p>
        </div>
      </div>
    );
  ...
  const getIntentBadge = (intent: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${styles[intent as keyof typeof styles]}`}
      >
        {intent === "buy"
          ? "🏠 Buying"
          : intent === "rent"
