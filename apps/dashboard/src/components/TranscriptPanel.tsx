...
const TranscriptPanel: React.FC<TranscriptPanelProps> = ({
  ...
  const getStatusBadge = () => {
    switch (callStatus) {
      ...
      case "completed":
        return (
          <span className="text-gray-600 text-sm font-medium">Call Ended</span>
        );
      case "missed":
        return (
          <span className="text-red-600 text-sm font-medium">Missed Call</span>
        );
    }
  };
        ...
        {transcript.length === 0 ? (
          ...
          <>
            {transcript.map((message) => (
              ...
              >
                <div
                  className={`max-w-[80%] ${message.speaker === "agent" ? "order-1" : "order-2"}`}
                >
                  {/* Speaker Label */}
                  <div
